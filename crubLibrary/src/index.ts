...
class Crublibrary {
  ...
  async update(txHash: string, value: any) {
    try {
      const response = await axios.put(
        `${this.apiUrl}/crud/update`, 
        JSON.stringify({ txHash, value }), 
        {
          headers: {
            'Authorization': `Bearer ${this.apiKey}`,
            'Content-Type': 'application/json'
...
async delete(txHash: string) {
  ...
  // Error handling
  private handleError(method: string, endpoint: string, error: any) {
